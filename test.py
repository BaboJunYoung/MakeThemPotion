...
class TestRecipes(unittest.TestCase):
    ...
    def test_order_does_not_matter(self):
        a = gl.get_potion_by_ingredients(["ginseng", "grape"])
        b = gl.get_potion_by_ingredients(["grape", "ginseng"])
        self.assertEqual(a["potion_id"], "healing")
        self.assertEqual(b["potion_id"], "healing")

    def test_three_ingredient_recipe(self):
        result = gl.get_potion_by_ingredients(["eyeball", "ginseng", "wing"])
        self.assertEqual(result["potion_id"], "legendary")

    def test_invalid_combo_is_failed(self):
        result = gl.get_potion_by_ingredients(["grape", "tomato"])
        self.assertEqual(result["potion_id"], "failed")
        self.assertEqual(result["ingredients"], ["grape", "tomato"])

    def test_recipe_combos_are_unique(self):
        combos = [tuple(sorted(r["ingredients"])) for r in gl.RECIPES]
...
class TestBrewingFlow(unittest.TestCase):
    ...
    def test_add_ingredient_rules(self):
        self.assertTrue(self.gs.add_ingredient("ginseng"))
        # 중복 불가
        self.assertFalse(self.gs.add_ingredient("ginseng"))
        self.assertTrue(self.gs.add_ingredient("grape"))
        # 2개여도 더 넣을 수 있다 (최대 3개)
        self.assertTrue(self.gs.add_ingredient("eyeball"))
        # 최대 3개 초과 불가
        self.assertFalse(self.gs.add_ingredient("wing"))
        self.assertEqual(len(self.gs.selected), 3)

    def test_unknown_ingredient_rejected(self):
        self.assertFalse(self.gs.add_ingredient("dragon_egg"))

    def test_clear(self):
        self.gs.add_ingredient("ginseng")
        self.gs.clear_ingredients()
        self.assertEqual(self.gs.selected, [])
        self.assertEqual(self.gs.phase, gl.PHASE_ORDER)

    def test_remove_ingredient_cancels(self):
        self.gs.add_ingredient("ginseng")
        self.gs.add_ingredient("grape")
        # 다시 누른 재료만 취소되고 나머지는 남는다
        self.assertTrue(self.gs.remove_ingredient("ginseng"))
        self.assertEqual(self.gs.selected, ["grape"])
        self.assertEqual(self.gs.phase, gl.PHASE_BREWING)
        # 선택 안 된 재료는 취소 불가
        self.assertFalse(self.gs.remove_ingredient("eyeball"))
        # 마지막 하나까지 빼면 주문 단계로 돌아온다
        self.assertTrue(self.gs.remove_ingredient("grape"))
        self.assertEqual(self.gs.phase, gl.PHASE_ORDER)

    def test_remove_undoes_craft(self):
        self.gs.add_ingredient("ginseng")
        self.gs.add_ingredient("grape")
        self.gs.craft()
        self.assertEqual(self.gs.phase, gl.PHASE_CRAFTED)
        self.gs.remove_ingredient("grape")
        self.assertIsNone(self.gs.crafted_potion)
        self.assertEqual(self.gs.phase, gl.PHASE_BREWING)
...
class TestFullDayProgression(unittest.TestCase):
    ...
    def test_failed_day_retries(self):
        ...
        for _ in range(gs.total_customers):
            gs.add_ingredient("grape")
            gs.add_ingredient("tomato")  # 어떤 레시피도 아님
            gs.craft()
